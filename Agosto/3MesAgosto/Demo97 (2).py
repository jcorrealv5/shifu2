...
class Dialogo(QDialog):
    ...
    def obtenerAtributosSeleccionados(self):
        lstAtributos=[]
        for i in range(self.lstAtributos.count()):
            if self.lstAtributos.item(i).checkState() == Qt.Checked:
               lstAtributos.append(i)
        return lstAtributos
    ...
    def buscarAtributo(self):
        ...
        self.atributo = self.obtenerAtributosSeleccionados()# self.lstAtributos.currentIndex()
    ...
    def run(self):
        ...
        for i in range(self.nArchivos):
            ...
            with torch.no_grad():
                ...
                for attr in self.atributo:
                    rpta = salida[0][attr].item()
                ...
                ##validacion para que la foto cumpla con todas las caracteristicas
                ''' 
                tieneAtributo=True
                for attr in self.atributo:
                    rpta = salida[0][attr].item()
                    tieneAtributo = (rpta>0.5)
                    if(tieneAtributo==False):
                        break  
                if  tieneAtributo:
                    c = c + 1
                    self.sleep(1)
